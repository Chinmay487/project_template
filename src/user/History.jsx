...
import { Grid, Typography, Box, useTheme, Button , CircularProgress} from '@mui/material';
            ...
            <Grid container sx={{ boxShadow: theme.shadows[5], backgroundColor: '#F5F5F5' }} >
                <Grid item md={3} sm={12} xs={12} >
                    <Box component="center">
                        <Box component="img" src={props.item.thumbnail} sx={{ maxWidth: { lg: "80%", md: "80%", sm: "80%", xs: "80%" } , height:'10rem'}} />
                    </Box>
                </Grid>
                <Grid item md={7} sm={12} xs={12} sx={profileGridItem} >
                    ...
                    <Typography sx={profileGridItemText} variant="h6"> Price : {props.item.price} </Typography>
                    {
                        ...
                        </Box> : <Typography sx={profileGridItemText} variant="h6"> Qty : {props.item.quantity} </Typography>
                    }
                </Grid>
                <Grid item md={2} sm={12} xs={12} sx={profileGridItem}>
                    {
                        (props.is_cart || props.isSeller) ?
                            <Box sx={cartButtons}>
                                ...
                                <Button variant='text' onClick={deleteProduct} color='error' sx={{ width:`${props.isSeller ? '100%' : '30%'}`,height : `${props.isSeller ? "auto":"2rem"}` }}>
